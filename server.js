...
app.put("/api/agenda/settings", requireAuth, requireRole("admin"), async (req, res) => {
  ...
  await pool.query(
    ...
    [baseUrl ? baseUrl.replace(/\/+$/, "") : null, JSON.stringify(slugMap || {}), req.user.email]
  );
  res.json({ ok: true });
});
...
app.post("/api/agenda/block-slot", requireAuth, async (req, res) => {
  ...
  const baseUrl = settingsRows[0] && settingsRows[0].agenda_base_url;
...
app.post("/api/agenda/remove-action", requireAuth, async (req, res) => {
  ...
  const baseUrl = rows[0] && rows[0].agenda_base_url;
