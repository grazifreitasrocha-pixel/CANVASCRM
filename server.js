...
async function initDb() {
  ...
  await pool.query(`
    ...
    WHERE id = 1 AND (origens_lead IS NULL OR origens_lead = '[]'::jsonb)
  `, [JSON.stringify([
    { id: "ol-1", nome: "Indicação" },
    { id: "ol-2", nome: "Instagram" },
    { id: "ol-3", nome: "Google" },
    { id: "ol-4", nome: "Site" },
...
app.put("/api/users/:id", requireAuth, requireRole("admin"), async (req, res) => {
  ...
  if (req.params.id === req.user.id && role && role !== req.user.role) {
    return res.status(400).json({ error: "Você não pode alterar o papel da sua própria conta." });
  }
  if (role && !ROLES.includes(role)) {
    return res.status(400).json({ error: "Papel inválido." });
  }
  if (password && password.length > 0 && password.length < 6) {
    return res.status(400).json({ error: "A senha precisa ter pelo menos 6 caracteres (ou deixe em branco para não alterar)." });
  }
  try {
    const fields = [];
    const values = [];
...
// ---- Relatório de negociações paradas (follow-up) + e-mail diário pro vendedor ----
...
function diasParado(dataISO) {
  if (!dataISO) return 0;
  const then = new Date(dataISO + (dataISO.length === 10 ? "T00:00:00" : ""));
  if (isNaN(then.getTime())) return 0;
  const now = new Date();
  return Math.max(0, Math.floor((now - then) / (1000 * 60 * 60 * 24)));
}

// Busca todo negócio em aberto sem mudar de etapa há mais de X dias, agrupado por vendedor.
async function buscarNegociacoesPatadas(diasMinimo = 1) {
  const { rows } = await pool.query("SELECT data FROM clients");
  const abertos = rows.map((r) => r.data).filter((c) => c.status === "aberto");
  const paradas = abertos
    .map((c) => ({
      client: c,
      dias: diasParado(c.stageUpdatedAt || c.createdAt),
    }))
    .filter((x) => x.dias >= diasMinimo);

  const porVendedor = {};
  for (const item of paradas) {
    const vid = item.client.vendedorUserId;
    if (!vid) continue;
...
function montarEmailHtml(nomeVendedor, itens) {
  const linhas = itens
    .sort((a, b) => b.dias - a.dias)
    .map(
      (i) =>
        `<tr>
          <td style="padding:8px 12px;border-bottom:1px solid #E4DFD1;">${i.client.razaoSocial || "Sem nome"}</td>
          <td style="padding:8px 12px;border-bottom:1px solid #E4DFD1;">${i.client.stage || "-"}</td>
          <td style="padding:8px 12px;border-bottom:1px solid #E4DFD1;text-align:right;color:${i.dias >= 3 ? "#A8483E" : "#8A6524"};font-weight:700;">${i.dias} dia${i.dias === 1 ? "" : "s"}</td>
        </tr>`
    )
    .join("");
  return `
    <div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;max-width:600px;margin:0 auto;">
      <h2 style="color:#152E25;">Follow-up de hoje</h2>
      <p style="color:#5A5548;">Oi, ${nomeVendedor}! Estas negociações estão sem movimentação há 1 dia ou mais:</p>
      <table style="width:100%;border-collapse:collapse;font-size:14px;">
        <thead>
          <tr style="background:#F3F0E6;">
            ...
            <th style="padding:8px 12px;text-align:right;">Parado há</th>
          </tr>
        </thead>
        <tbody>${linhas}</tbody>
      </table>
...
async function enviarDigestDiario() {
  ...
  for (const vid of vendedorIds) {
    const { rows } = await pool.query("SELECT name, email FROM users WHERE id = $1", [vid]);
    ...
    try {
      ...
      console.error(`Erro ao enviar e-mail de follow-up pra ${email}:`, err.message);
    }
  }
...
app.get("/api/reports/stale-negotiations", requireAuth, async (req, res) => {
  ...
  const nomes = Object.fromEntries(rows.map((u) => [u.id, u.name]));
  const lista = Object.entries(porVendedor).flatMap(([vid, itens]) =>
    itens.map((i) => ({
      clientId: i.client.id,
      razaoSocial: i.client.razaoSocial,
      stage: i.client.stage,
      dias: i.dias,
      vendedor: nomes[vid] || "-",
    }))
  );
  lista.sort((a, b) => b.dias - a.dias);
  res.json({ emailConfigured: EMAIL_CONFIGURED, negociacoes: lista });
});
...
// Roda a cada hora; dispara o resumo diário uma única vez por dia, às 8h (horário do servidor).
setInterval(async () => {
  if (!EMAIL_CONFIGURED) return;
  const now = new Date();
  if (now.getHours() !== 8) return;
  try {
    const { rows } = await pool.query("SELECT last_stale_digest_sent_date FROM integration_settings WHERE id = 1");
    const jaEnviouHoje = rows[0] && rows[0].last_stale_digest_sent_date && new Date(rows[0].last_stale_digest_sent_date).toDateString() === now.toDateString();
    if (jaEnviouHoje) return;
    await enviarDigestDiario();
    await pool.query("UPDATE integration_settings SET last_stale_digest_sent_date = CURRENT_DATE WHERE id = 1");
  } catch (err) {
    console.error("Erro no envio automático do follow-up diário:", err);
  }
}, 60 * 60 * 1000);

// ---- Origem do lead (separado da precificação -- comercial também precisa acessar) ----

app.get("/api/origens-lead", requireAuth, async (req, res) => {
  const { rows } = await pool.query("SELECT origens_lead FROM integration_settings WHERE id = 1");
  res.json({ origens: (rows[0] && rows[0].origens_lead) || [] });
});

app.put("/api/origens-lead", requireAuth, requireRole("admin", "comercial"), async (req, res) => {
  const { origens } = req.body;
  await pool.query(
...
app.post("/api/agenda/webhook", async (req, res) => {
  ...
  try {
    ...
    const { rows: userRows } = await pool.query("SELECT name FROM users WHERE id = $1", [vendedorUserId]);
    const vendedorNome = userRows[0] ? userRows[0].name : "";

    // Avisa a pessoa
    const message = `Novo agendamento: ${guest_name || "alguém"} marcou um horário com você em ${date} às ${start_time}.${notes ? " Obs: " + notes : ""}`;
    await pool.query("INSERT INTO notifications (user_id, message, client_id) VALUES ($1, $2, $3)", [vendedorUserId, message, null]);

    // Cria um card novo no funil de vendas pra essa reunião -- entra em "Contatado" (já que a
    // pessoa tomou a iniciativa de marcar o horário) com a reunião já em "Próxima ação".
    const newClientId = "agenda-" + Date.now() + "-" + Math.random().toString(36).slice(2, 8);
    const nowISO = new Date().toISOString();
    const newClient = {
      id: newClientId,
      razaoSocial: guest_name || "Lead da Agenda",
      nomeFantasia: "",
      cnpj: "",
      cnpjsAdicionais: [],
      telefone: "",
      ...
      responsavelIntegracaoUserId: "",
      tipoEntrada: "",
      proximaAcaoData: date || "",
      proximaAcaoHora: start_time || "",
      proximaAcaoTipo: "Reunião",
      proximaAcaoDescricao: `Reunião agendada pelo link — ${guest_name || "cliente"}`,
      proximaAcaoResponsavelId: "",
      proximaAcaoGoogleEventId: "",
      proximaAcaoAgendaBookingId: "",
      proximaAcoesHistorico: [],
      validadeProposta: "",
      dataVenda: "",
      ...
      notas: [
        {
          ...
          text: `Lead criado automaticamente: ${guest_name || "-"} (${guest_email || "sem e-mail"}) agendou uma reunião pelo link público para ${date} às ${start_time}.${notes ? " Observações: " + notes : ""}`,
        },
      ],
      stage: "Contatado",
      status: "aberto",
      onboardStage: null,
      onboardStageUpdatedAt: "",
      stageUpdatedAt: nowISO.slice(0, 10),
    ...
    await pool.query("INSERT INTO clients (id, data, updated_by) VALUES ($1, $2, 'agenda-webhook')", [newClientId, JSON.stringify(newClient)]);
  } catch (err) {
