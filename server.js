...
app.post("/api/agenda/webhook", async (req, res) => {
  ...
  try {
    ...
    // Só avisa a pessoa -- não cria mais um card automaticamente no funil, porque nem toda
    // reunião marcada pelo link é um lead novo (pode ser cliente já existente, reunião interna,
    // fornecedor, etc.). Quem vende decide se vale a pena cadastrar como negociação.
    const message = `Novo agendamento: ${guest_name || "alguém"} (${guest_email || "sem e-mail"}) marcou um horário com você em ${date} às ${start_time}.${notes ? " Obs: " + notes : ""}`;
